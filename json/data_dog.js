var data_dog = [
   ...
   },
   {
      ...
      "Size": "Medium", "Character": "Hypoallergenic",
      "Color": "Black",
      "Geography": "Europe",
      "Origin": "Switzerland",
   ...
   },
   {
      ...
      "Name": "Hamilton Hound",
      "Size": "Large", "Character":
         "Smartest", "Color": "Others",
      "Geography": "Europe",
      "Origin": "United States",
      "Breed_Group": "Working (AKC:1937), Guardian Dogs (UKC)",
      "Type": "Purebred",
   ...
   },
   {
      ...
      "Overview": "The Jämthund is a large, rectangular spitz breed, clean cut, strong, substantial and agile. It must not give the impression of having a long body or of being overly heavy in body, as it is a breed known for great endurance. Courageous and energetic, but also stoically calm. \n As with most breeds developed for hunting, the Jämthund requires a lot of regular exercise to stay fit, both physically and mentally. It quickly becomes bored if kept indoors for too long and can become destructive.",
      "History": "The Jämthund is a breed with ancient origins, though it was only recognized as a separate breed in 1946, because it used to be shown together with the smaller Norwegian Elkhound. The breed was mainly used for hunting Elk, though it was sometimes used on bear and lynx as well.",
   ...
   },
   {
      ...
      "Overview": "The Shiba Inu is moderately compact, being slightly longer than it is tall. It has typical traits of dogs from Northern heritage: small erect ears, thick fur, powerful body and curled tail. Its expression is bold, spirited and good-natured. The gait is light, quick and agile, with an effortless, smooth stride. The double coat consists of a strong straight outer coat with a soft undercoat, imparting great insulation. These traits enabled the shiba to hunt small game through dense cover.",
   ...
   {
      "Pic": "Whippet.jpg", "Name": "Whippet", "Size": "Medium", "Character": "Hypoallergenic", "Color": "Black",
      "Geography": "Europe",
      "Origin": "United Kingdom",
   ...
   {
      "Pic": "Basenji.jpg", "Name": "ABasenji", "Size": "Medium", "Character": "Fluffy", "Color": "Others",
      "Geography": "Asia",
      "Origin": "Congo",
      ...
      "History": "The Basenji (also known as an “African Bush Dog”, “Congo Dog”, or “Congo Terrier”) is an ancient breed which can be seen depicted on stone tablets in Egyptian tombs. The modern breed originates in the African Congo (“Basenji” means “bush thing”), where they were used as pack hunters to chase game into nets. \n Basenjis were imported to England in the 1930’s, then to America. They increased in popularity after being featured as the title character of a popular 1950’s book and movie, “Goodbye, My Lady”. Some Basenjis from Africa were imported in the 1980’s, widening the gene pool and introducing the brindle color to the breed.",
      "Galary": "G_Basenji.jpg"
   },
   {
      "Pic": "ThaiRidgeback.jpg", "Name": "Thai Ridgeback", "Size": "Medium", "Character": "Smartest", "Color": "Black",
      "Geography": "Asia",
      "Origin": "Thailand",
   ...
   {
      "Pic": "Boxer.jpg", "Name": "Boxer", "Size": "Small", "Character": "Best guard", "Color": "Others", "Geography": "Europe",
      "Origin": "Germany",
   ...
   {
      "Pic": "MiniatureSchnauzer.jpg", "Name": "Miniature Schnauzer", "Size": "Small", "Character": "Smartest", "Color": "Black",
      "Geography": "Europe",
      "Origin": "Germany",
   ...
   {
      "Pic": "AmericanStaffordshireTerrier.jpg", "Name": "American Staffordshire Terrier", "Size": "Medium", "Character": "Smartest", "Color": "Others",
      "Geography": "North America",
      "Origin": "United States",
   ...
   {
      "Pic": "AiredaleTerrier.jpg", "Name": "Airedale Terrier", "Size": "Medium", "Character": "Smartest", "Color": "Others",
      "Geography": "Europe",
      "Origin": "United Kingdom",
...
var start = 0;
var end = perPage;
var totalPage = Math.ceil(data_dog.length / perPage);
...
function filter() {
   const size = $('input[name=sizeOp]:checked').val();
   const character = $('input[name=characterOp]:checked').val();
   const color = $('input[name=colorOp]:checked').val();
   const geography = $('input[name=GeographyOp]:checked').val();
   let tmp_data = JSON.parse(JSON.stringify(data_dog)) // clone data_dog
   if (size)
      tmp_data = tmp_data.filter(dog => {
         return dog.Size == size
      });
   if (character)
      tmp_data = tmp_data.filter(dog => {
         return dog.Character == character
      });
   if (color)
      tmp_data = tmp_data.filter(dog => {
   ...
   if (geography)
      tmp_data = tmp_data.filter(dog => {
         return dog.Geography == geography
      });
...
function renderDataDog() {
   ...
   html = '';
   let filtered_dog = filter() || data_dog;
   if (filtered_dog.length == 0) {
   ...
   const content = filtered_dog.map((item, index) => {
      if (index >= start && index < end) {
         html += '<div class = "content_item">';
         html += '<figure class ="column">';
         html += '<img src =img/dog/' + item.Pic + '>';
...
function renderListPage(data) {
   ...
   let filtered_dog = data || data_dog;
   totalPage = Math.ceil(filtered_dog.length / perPage);
   let html = '';
